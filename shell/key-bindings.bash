...
__fzf_myselect__() {
  ...
  #  -o -type l -print 2> /dev/null | cut -b3-"}"
  #eval "$cmd" | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_CTRL_T_OPTS" fzf -m "$@" | while read -r item; do
  $HOME/.fzf/bin/olsone | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_CTRL_T_OPTS" fzf --ansi -m "$@" | while read -r item ; do
    printf '%q ' "$item"
  done
  echo
}
...
__fzf_myselect_cd__() {
  ...
  #  -o -type l -print 2> /dev/null | cut -b3-"}"
  #eval "$cmd" | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_CTRL_T_OPTS" fzf -m "$@" | while read -r item; do
  olsone | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_CTRL_T_OPTS" fzf --ansi -m "$@" | while read -r item ; do
    if test ! $item ;then
      echo "."
    else
      printf '%q ' "$item"
